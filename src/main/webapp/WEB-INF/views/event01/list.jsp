			...
			<div id="container">
				...
				<div id="outbox">
					...
					<div id="contents">
						<div id="customer">
							<h2>
								<strong>이벤트 게시판</strong><span>쟈뎅샵 소식을 전해드립니다.</span>
							</h2>
								...
								<table summary="NO, 제목, 등록일, 조회수 순으로 공지사항을 조회 하실수 있습니다."
									...
									<tbody>
										...
										<c:forEach items="${map.list}" var="boardVo">
											<tr>
												<td class="tnone">1</td>
												<td class="left"><a href="view?bid=${boardVo.bid}">${boardVo.btitle}</a> <img
													src="../images/ico/ico_new.gif" alt="NEW" /></td>
												<td>${boardVo.bdate}</td>
												<td class="tnone right">${boardVo.bhit}</td>
											</tr>
										</c:forEach>
										<!--  -->
									</tbody>
							...
							<div class="btnAreaList">
								...
								<div class="allPageMoving1">
									...
									<ul class="page-num">
										...
										<c:if test="${map.page != 1 }">
											<a
												href="boardList?page=1&category=${map.category}&searchWord=${map.searchWord}"><li
												class="first"></li></a>
										</c:if>
										...
										<c:if test="${map.page > 1 }">
											<a
												href="boardList?page=${map.page-1}&category=${map.category}&searchWord=${map.searchWord}"><li
												class="prev"></li></a>
										</c:if>
										...
										<c:forEach var="nowpage" begin="${map.startpage}"
											...
											<c:if test="${map.page != nowpage }">
												<a
													href="list?page=${nowpage }&category=${map.category}&searchWord=${map.searchWord}"><li
													class="num"><div>${nowpage}</div></li></a>
											</c:if>
										</c:forEach>
										...
										<c:if test="${map.page < map.endpage }">
											<a
												href="List?page=${map.page+1}&category=${map.category}&searchWord=${map.searchWord}"><li
												class="next"></li></a>
										</c:if>
										...
										<c:if test="${map.page != map.maxpage }">
											<a
												href="boardList?page=${map.maxpage }&category=${map.category}&searchWord=${map.searchWord}"><li
												class="last"></li></a>
										</c:if>
								</div>
								<!-- //페이징이동1 -->
