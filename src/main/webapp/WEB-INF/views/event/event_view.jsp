...
<script type="text/javascript">
  function replyBtn(){
	  ...
	  if($("#replyType").val().length<=2){
		  alert("2글자 이상 입력하셔야 댓글을 등록할수 있습니다.");
		  $("#replyType").focus();
		  return false;
	  }
	  ...
	  $.ajax({
			...
			success:function(data){   //data.eventVo, data.replyCount 댓글저장후 1개검색,댓글개수
			   var html="";
			   html += '<ul id="'+data.eventVo.cno+'">';
			   html += '<li class="name">'+data.eventVo.id;
			   html += '<span>&nbsp;['+data.eventVo.cdate+']</span></li>';
			   <!-- 자신이 쓴글인지 확인 -->
			   if(s_id == data.eventVo.id){
				   ...
				   if(data.eventVo.cpw ==null || data.eventVo.cpw == ""){
					   html += '<li class="txt">'+data.eventVo.ccontent+'</li>';
				   }else{
					   html += '<li class="txt">';
					   html += '<a href="#" class="passwordBtn"><span class="orange">※ 비밀글입니다.</span></a>';
					   html += '</li>';
				   }
			   }
			   html += '</ul>';
			...
			error:function(){  //실패시
				alert("error");
					...
					<div class="replyWrite">
						<ul>
							<li class="in">
								<p class="txt">총 <span class="orange">${map.replyCount}</span> 개의 댓글이 달려있습니다.</p>
								<p class="password">비밀번호&nbsp;&nbsp;<input type="password" id="replynum" class="replynum" /></p>
								<textarea id="replyType" class="replyType"></textarea>
							</li>
					...
					<div class="replyBox">
						...
						<ul id="${cno}">
							<li class="name">${eventVo.id}
							<span>[${eventVo.cdate }]</span></li>
							<!-- 자신이 쓴글인지 확인 -->
							...
							<c:if test="${session_id != eventVo.id }">
							    <!-- 비밀번호가 들어가 있는지 확인 -->
							    <c:if test="${eventVo.cpw ==null or eventVo.cpw == '' }">
							      <li class="txt">${eventVo.ccontent }</li>
							    </c:if>
							    <c:if test="${eventVo.cpw != null }">
								    <li class="txt">
